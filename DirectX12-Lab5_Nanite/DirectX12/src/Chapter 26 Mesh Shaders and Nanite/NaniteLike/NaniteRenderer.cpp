...
void NaniteRenderer::Render(ID3D12GraphicsCommandList* cmdList, const Camera& camera,
    D3D12_CPU_DESCRIPTOR_HANDLE rtv, D3D12_CPU_DESCRIPTOR_HANDLE dsv, float totalTime)
{
    if (mTotalMeshlets == 0) return;
    
    if (mUseMeshShaders && mMeshShaderPSO)
        RenderMeshShader(cmdList, camera, rtv, dsv, totalTime);
    else
        RenderFallback(cmdList, camera, rtv, dsv, totalTime);
}
