...
{
    // Vertex animation: same sine wave as Mesh Shader path (Y wave in object space)
    const float animAmplitude = 0.08;
    const float animFreq = 2.0;
    const float animWaveScale = 4.0;
    float wave = sin(gTotalTime * animFreq + vin.PosL.x * animWaveScale) * animAmplitude;
    float3 posL = vin.PosL + float3(0.0, wave, 0.0);
