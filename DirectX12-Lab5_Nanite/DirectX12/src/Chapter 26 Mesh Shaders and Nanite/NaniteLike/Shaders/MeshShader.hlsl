...
cbuffer PassConstants : register(b0)
{
    ...
    float gTotalTime;         // For vertex animation
    uint2 gPadding2;
};
    ...
    out indices uint3 tris[MAX_MESHLET_PRIMITIVES])
{
    ...
    // Process vertices (with vertex animation — wave in object space)
    if (gtid < meshlet.VertexCount)
    {
        uint vertexIndex = UniqueVertexIndices[meshlet.VertexOffset + gtid];
        Vertex v = Vertices[vertexIndex];
        
        // Vertex animation: sine wave along Y (amplitude and frequency in object space)
        const float animAmplitude = 0.08;
        const float animFreq = 2.0;
        const float animWaveScale = 4.0;
        float wave = sin(gTotalTime * animFreq + v.Position.x * animWaveScale) * animAmplitude;
        float3 posL = v.Position + float3(0.0, wave, 0.0);
