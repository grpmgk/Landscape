...
private:
    ...
    // Terrain painting (brush by terrain color)
...
private:
    ...
    // Paint texture for brush-on-terrain (like Unreal landscape paint)
    ...
    float mBrushSize = 30.0f;
    XMFLOAT3 mPaintColor = { 1.0f, 0.0f, 0.0f };
};
...
void TerrainApp::Draw(const GameTimer& gt)
{
    ...
    // Update paint texture on GPU if modified
...
void TerrainApp::HandleKeyboard(const GameTimer& gt)
{
    ...
    // Paint color: R / G / B
    static bool rKey = false, gKey = false, bKey = false;
    if (GetAsyncKeyState('R') & 0x8000)  { if (!rKey) { mPaintColor = { 1.0f, 0.0f, 0.0f }; rKey = true; } }
    else rKey = false;
    if (GetAsyncKeyState('G') & 0x8000)  { if (!gKey) { mPaintColor = { 0.0f, 1.0f, 0.0f }; gKey = true; } }
    else gKey = false;
    if (GetAsyncKeyState('B') & 0x8000)  { if (!bKey) { mPaintColor = { 0.0f, 0.0f, 1.0f }; bKey = true; } }
    else bKey = false;
}
...
void TerrainApp::BuildRootSignature()
{
    ...
    texTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 4, 0, 0); // 4: height, diffuse, normal, paint
...
void TerrainApp::BuildDescriptorHeaps()
{
    ...
    srvHeapDesc.NumDescriptors = 4; // height, diffuse, normal, paint
    srvHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
    srvHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
    ThrowIfFailed(md3dDevice->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&mSrvHeap)));

    // Paint texture (512x512 RGBA) - created first so we can upload before other textures
    const UINT paintWidth = 512;
    const UINT paintHeight = 512;
    mPaintData.resize(paintWidth * paintHeight, 0x00000000);
...
void TerrainApp::PaintOnTerrain(const XMFLOAT3& worldPos)
{
    ...
    const int paintWidth = 512;
    const int paintHeight = 512;
    const int centerX = (int)(u * (paintWidth - 1));
    const int centerY = (int)(v * (paintHeight - 1));
    const int brushRadius = max(2, (int)(mBrushSize * paintWidth / terrainSize));
        ...
        for (int x = centerX - brushRadius; x <= centerX + brushRadius; ++x)
        {
            if (x >= 0 && x < paintWidth && y >= 0 && y < paintHeight)
            {
                ...
                if (distance <= (float)brushRadius)
                {
                    ...
                    const int index = y * paintWidth + x;
